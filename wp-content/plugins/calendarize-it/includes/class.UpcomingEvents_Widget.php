...
class UpcomingEvents_Widget extends WP_Widget {
	...
	function get_shortcode($widget_args,$sel){
		...
		foreach(array('premiere', 'loading_method', 'template', 'post_type','calendar','venue','organizer','taxonomy','terms','auto','horizon','number','showimage','words','dayspast','fcdate_format','fctime_format','calendar_url','specific_date','specific_date_end') as $field ){
...
//echo $sc;
	...
	function get_upcoming($widget_args,$sel){
		...
		foreach(array('premiere', 'loading_method', 'template', 'post_type','calendar','venue','organizer','taxonomy','terms','auto','horizon','number','showimage','words','fcdate_format','fctime_format','calendar_url','specific_date','specific_date_end') as $field ){
			$$field = isset($widget_args[$field])?$widget_args[$field]:'';
			//echo $field.": ".$$field."<BR />";
		}		
...
//		echo "</prE>";
	...
	function update( $new_instance, $old_instance ) {
		$instance = array();
		foreach(array('taxonomy', 'terms', 'loading_method', 'template', 'post_type','calendar','venue','organizer','auto','premiere','title','fcdate_format','fctime_format','horizon','number','showimage','words','dayspast','calendar_url','specific_date','specific_date_end') as $field){
	...
	function form( $instance ) {
		...
		foreach(array('taxonomy'=>'', 'terms'=>'', 'loading_method'=>'server', 'calendar_url'=>'', 'auto'=>0, 'premiere'=>0,'title'=>'','horizon'=>'hour','number'=>5,'showimage'=>0,'words'=>10, 'dayspast'=>'', 'fcdate_format'=>'MMM d, yyyy','fctime_format'=>'h:mmtt', 'specific_date'=>'', 'specific_date_end'=>'') as $field =>$default){
...
<?php endforeach;?>
	...
	<div class="" style="margin-top:10px;">
		...
		<select id="<?php echo $this->get_field_id('horizon')?>" name="<?php echo $this->get_field_name('horizon')?>" class="widefat">
			...
			<option value="end" <?php echo $horizon=='end'?'selected="selected"':''?> ><?php _e('Ending hour','rhc')?></option>			
