...
var ShareThisWidget = new Class({
    ...
    top:        0,
    hiddenEmbeds: false,
    loaderHTML: '<html><head><style>body{background:#E4ECF3 url(http://button.kadampa.org/load.gif) center center no-repeat;color:#638AA9;font:bold 18px Arial,Helvetica,sans-serif;padding:50px 20px;text-align:center;}</style></head><body>Loading...</body></html>',
    initialize: function(options){
...
var ShareThis = new Class({
    ...
    initialize: function(options){
        ...
        var css = 'http://button.kadampa.org/nkt-ikbu.css';
        this.load('link', {
    ...
    addEntry: function(properties, options){
        ...
        s.popup = function(e){
             ...
             SHARETHIS.selection();
             var p = window.open("loading.html", "stframe","status=1,toolbar=0,width=354,height=437");
             s.widget.popup = p;
             try {
                 p.document.open();
                 p.document.clear();
        ...
        if (typeof(s.options.button) == "undefined" || Boolean(s.options.button) != false) {
            ...
            a.title = "The New Kadampa Tradition - International Kadampa Buddhist Union";
            a.href = "#";
            a.onclick = (Boolean(s.options.popup) == true ? s.popup : s.onClick);
            var t = document.createElement("span");
            t.className = 'nktbuttontext';
    ...
    form: function(olist) {
        ...
        //f.className  = "stform";
        f.method = "post";
        f.action = "http://button.kadampa.org";
        f.target = "stframe";
    ...
    log: function(event, obj) {
        var url = "http://kadampaweb.org?event=" + event
...
_query.publisher = '3404c942-a71d-459b-a0cd-d79d39e1ab77';
